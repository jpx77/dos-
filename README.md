...
- Dependencias listadas en `requirements.txt` (incluye soporte de color con [Colorama](https://pypi.org/project/colorama/))
...
### Tema azul estilo TI-89

La interfaz emplea un esquema de azules claros con texto brillante para remitir a la pantalla de una TI-89. Si necesita desactivar los colores ANSI (por ejemplo, al redirigir la salida a un archivo o en terminales sin soporte de color), puede iniciar la consola con:

```bash
python -m ti89_calculator --no-color
```
...
Valores propios (λ: multiplicidad)
==================================
⎡⎛3, 1⎞, ⎛1, 1⎞⎤
⎣⎝   ⎠  ⎝   ⎠⎦
```
...
Para distribuir la calculadora como programa autónomo en Windows puede usar [PyInstaller](https://pyinstaller.org/):
...
El ejecutable se almacenará en `dist/ti89_calculator.exe` junto con los archivos auxiliares que PyInstaller requiera. Si desea añadir más opciones (por ejemplo, íconos personalizados), páselas como argumentos adicionales al script (`python scripts/build_executable.py --icon=mi_icono.ico`).
...
- `cli.py` implementa la interfaz de línea de comandos con un tema azul brillante similar a la TI-89.
